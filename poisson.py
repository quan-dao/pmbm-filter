...
class PointPoissonProcess(object):
    ...
    def give_birth(self, measurements: List[ObjectDetection], birth_per_meas=3) -> None:
        ...
        for meas in measurements:
            delta_x = np.random.uniform(-5, 5, size=birth_per_meas)
            delta_y = np.random.uniform(-5, 5, size=birth_per_meas)
            delta_yaw = np.random.uniform(-np.pi, np.pi, size=birth_per_meas)
    ...
    def update(self) -> None:
        for component in self.intensity:
            component['w'] += np.log(self.prob_detection)
